...
Everything betwee \code{/*} and \code{*/} is a comment, as well as everything between \code{//} and the end of the line.  If a comment block starts with \code{/**}, but not with \code{/***}, then the comment is added as a description to the first thing after that comment block that can have a description.  Currently, only procedures can have a description.
...
An object with name \code{MyName} declared in namaespace \code{MySpace} can be reffered to \code{MySpace::MyName}.  Inside \code{MySpace}, \code{MyName} can simply be refered to by \code{MyName}.

A Namespace can contain namespaces, vocabularies, theories, structures, procedures, optionas and using statements.  A using statement is of one of the following forms
\begin{lstlisting}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Symbol delcarations}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\label{ssec:symbols}
...
The global namespace contains a fixed vocabulary \code{std}, withc is defined as follows:
...
\end{lstlisting}
A theory contains sentences, inductive definitions, and fixpoint definitions.
...
A definition in \idp syntax consists of a set of rules, enclosed by `{\tt \{}' and `{\tt \}}'. Each rule ends with a `{\tt .}'. The definitional implication $\rul$ is written `{\tt <-}'. The quantifications before the head may be omitted in \idp, i.e., all free variables of a rule are implicitly universally quantified. If the body of a rule is empty, the rule symbol `{\tt <-}' can be omitted. Recursive definitions are allowed in \idp. \todo{something about semantics?}
...
\[ \sum_{i | I\vDash {\tt phi\_i}} {\tt t_i }. \]
	...
	\[ \textrm{max}(\{ {\tt t} \mid {\tt(a\_1,a\_2,..., a\_n)} | I\vDash {\tt phi} {\tt t },\}) \] for sets of the second sort.  Sets of the first sort are handled analogously. %{\tt(a\_1,a\_2,..., a\_n)} \in S} {\tt a\_1 }, \]
...
\todo{DIT KAN TOCH NIET MEER, of wel?}

If a function $F$ is explicitely declared partial, one can specify a domain for which it is total in the form of a formula $\varphi$. Then, for all input arguments for which $\varphi$ is true, and only for these, $F$ has an output. To declare the domain of a partial function, the keyword `{\tt domain}' can be used, as is shown in the following example. 

\subsection{Fixpoint definitions}
\todo{nog niet getest. ik vermoed dat er hier niets over komt}
...
Since built-in functions are overloaded...\todo{stukje over wat typederivation precies doet voor gelijkheid en zo.. Broes?}

First consider the case where a variable occurs in typed positions with different types. The \idp system will then give a warning. If all the typed positions where the variable occurs have a common ancestor type $T$, then the variable is assigned this type $T$. If they do not have a common ancestor, but are all of an integer type, then the variable is assumed to range over all integers.


Now consider the case where a variable does not occur in a typed position. Then, the \idp system reports an error.

Because of the architecture of the system, problems can only be solved if all variables with an integer type range over a finite number of integers. Sometimes, however, if a variable ranges over all integers, only a finite interval of them is relevant. E.g., the constraint 
`{\tt ! x : 1 < x \& x < 5 => x ~= 10}' is certainly satisfied for all {\tt x}'s outside the interval $[2..4]$, so only this interval is relevant to look at. Therefore, this constraint does not yield an error message. It is beyond the scope of this manual to describe when the system can find a finite relevant interval and when it cannot. When it cannot, an error messages is returned.

To avoid these errors, one can add bounds: 
\begin{itemize}
	\item If {\tt x} is universally quantified, i.e., it occurs as `{\tt ! x : ...}', then write `{\tt ! x : phi(x) => ...}' instead, where {\tt phi} is a formula that can obviously only be true for a finite interval of {\tt x}'s. E.g., {\tt phi} is the formula `{\tt 1 < x \& x < 5}'.
	\item If {\tt x} is existentially quantified, then write `{\tt ? x : phi(x) \& ...}' instead of `{\tt ? x : ...}', where {\tt phi} is as above.
\end{itemize}
...
\end{lstlisting}
where {\tt MyType} is the name of the enumerated type and {\tt El\_1; El\_2; ... ; El\_n } are the names of the objects of that type. Names of objects can be (positive and negative) integers, strings, chars, compound domain elements, or identifiers that start with an upper- or lowercase letter. In the case all subtypes of a given type are specified, the supertype is derived to be the union of all elements of the subtypes.  If a type is not specified, all domain elements of that type that occur in a predicate or function interpretation (see below) are automatically added to that type.  
...
\section{Options}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo{stukje over: Hoe opties zetten en gebruiken?}

\todo{Opties die we niet supporten in commentaar zetten}
\begin{description}
	...
	\item[language = [asp, ecnf, idp, latex, tptp, txt]] The language used when printing objects. \todo{remove latex en zo.. die crashen}
	\item[longestbranch = [0..2147483647]] \todo{}
	\item[longnames = [false, true]] If true, everything is printed with reference to their vocabulary.  For example, a predicate \code{P} from vocabulary \code{V} will be printed \code{V::P} instead of \code{P}.
	\item[modelformat = [all, threevalued, twovalued]] \todo{wat doet dit?}
	\item[nbmodelequivalent = [false, true]] \todo{wat doet dit?}
	\item[nrmodels = [0..2147483647]] Set the number of models wanted from the modelexpansion inference.  If set to 0, all models are returned.
	...
	\item[symmetry = [0..2147483647]] \todo{?}
...
\todo{section over wat er nog komt?}
