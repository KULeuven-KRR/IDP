...
\begin{itemize}
	...
	\begin{lstlisting}
	theory T:V{
		! MyVar [MyType] : ? MyVar2 [MyType2] MyVar3 [MyType3] : MyPred(MyVar1,MyVar2,MyVar3).
	}
	\end{lstlisting}
	To specify that a variable should range over all integers, one can write `{\tt [int]}'.
...
\todo{
IMPORTANT: this section should be rewritten to the new type derivation system!!!}
Moet alleszins bevatten:
A predicate or function symbol can be disambiguated by scecifying its vocabluary and / or types.  E.g.,
...
\begin{itemize}
	\item an occurrence as argument of a predicate: $P(\ldots,x,\ldots)$;
	\item an occurrence as argument of a function: $F(\ldots,x,\ldots) = \ldots$;
	\item an occurrence as return value of a function: $F(\ldots) = x$ or $F(\ldots) \neq x$. 
	\item an occurrence as argument or return value of a arithmetic function. 
\end{itemize}
All others positions are untyped. 

Basically, if a variable occurs in a typed position, it gets the type of that position.  If the predicate or function symbol is overloaded, not type derivation is done.
...
\subsubsection{Chains of (in)equalities}
...
\todo{DIT KAN TOCH NIET MEER? wat hierna komt}
...
\todo{nog niet getest!! in plaatsen?}
