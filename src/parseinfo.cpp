...
TermParseInfo::TermParseInfo(const TermParseInfo& p) : ParseInfo(p.line(),p.col(),p.file()), _original(0) {
	if(p.original()) _original = p.original()->clone();
}
...
FormulaParseInfo::FormulaParseInfo(const FormulaParseInfo& p) : ParseInfo(p.line(),p.col(),p.file()), _original(0) {
	if(p.original()) _original = p.original()->clone();
}
...
SetParseInfo::SetParseInfo(const SetParseInfo& p) : ParseInfo(p.line(),p.col(),p.file()), _original(0) {
	if(p.original()) _original = p.original()->clone();
}
...
TermParseInfo::~TermParseInfo() {
	if(_original) _original->recursiveDelete();
}
