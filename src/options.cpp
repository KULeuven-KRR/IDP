...
Options::Options(bool verboseOptions): _isVerbosity(verboseOptions) {
	...
	if (verboseOptions) {
		...
		BoolPol::createOption(BoolType::XSB_SUBSUMPTIVE_TABLING, "xsbsubsumptivetabling", boolvalues, false, PrintBehaviour::DONOTPRINT); // Translation to XSB code does not maintain predicate and atom names, but introduces identifiers to minimize communication overhead with XSB
		...
		BoolPol::createOption(BoolType::GUARANTEE_NO_REC_AGG, "guaranteenorecursiveaggs", boolvalues, false, PrintBehaviour::DONOTPRINT);//Only for internal use for the moment: serves for avoiding loops in bootstrapping
