...
\begin{itemize}
  ...
  \item (optional) Gecode for constraint programming suppport.
\end{itemize}
...
An interactive interface using the files $files$ is started with \footnote{Currently, including additional files during an interactive session is not supported.}
\begin{lstlisting}
idp -i files
\end{lstlisting}
