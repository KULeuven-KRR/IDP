...
\end{itemize}
The following order of binding is used: $\lnot$ binds tightest, next $\land$ and $\lor$, then $\limplies$ and $\lequiv$, and finally $\forall$ and $\exists$. Desambiguation can be done using brackets `(' and `)'. E.g. the formula $\forall x\ P(x) \land \lnot Q(x) \limplies R(x)$ is equivalent to the formula $\forall x\ ((P(x) \land (\lnot Q(x))) \limplies R(x))$.
...
Syntacticly, there are two kinds of sets in \idp. 
\begin{itemize}
...
\end{itemize} 
The informal interpretation of sets of the different kinds is (respectively):
\begin{itemize}
 \item The multiset of all \code{t\_i} for which \code{phi\_i} is true (multiple occurences of the same term are possible).
...
\[ \sum_{i | I\vDash {\tt phi\_i}} {\tt t_i }. \]
	\item[Product:] Products are defined similar to sum.  The syntax for the product of \code{S} is \code{prod S}.
