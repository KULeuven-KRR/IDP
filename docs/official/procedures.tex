...
\begin{description}
	...
	\item[propagate(theory,structure)]
 		...
 		Returns nil when propagation results in an inconsistent structure.
 	\item[initialise(theory,structure)] can only be called with an LTC theory. Performs initialisation for the \href{http://dtai.cs.kuleuven.be/krr/files/progression.pdf}{progression inference} 
 	\item[progress(theory,structure)] can only be called with an LTC theory. Performs one progression step for the \href{http://dtai.cs.kuleuven.be/krr/files/progression.pdf}{progression inference} 
	\item[isinvariant(theory, theory)] uses a theorem prover (set by stdoptions.provercommand) to try to prove that the second theory is an invariant of the first theory. The second theory should be of the form : $\forall t[Time]:\varphi[t]$ and the first should be an LTC theory.
	\item[isinvariant(theory, theory, structure)] uses the model expander to prove that the second theory is an invariant of the first theory in the context of the given structure. The second theory should be of the form : $\forall t[Time]:\varphi[t]$ and the first should be an LTC theory.
