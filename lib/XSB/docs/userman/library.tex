...
\ourmoditem{delete\_ith(+Index, +List, ?Element, ?RestList)}{delete\_ith/4}{listutil}
    Succeeds if the ${\tt Index^{th}}$ element of the list {\tt List}
...
\ourmoditem{same\_length(?List1, ?List2)}{same\_length/2}{basics}
...
\ourmoditem{select(?Element, ?L1, ?L2)}{select/3}{basics}
...
\ourmoditem{member2(?Element, ?List)}{member2/2}{listutil}
...
\ourmoditem{load\_csv(+FileName,+PredSpec)}{load\_csv/2}{proc\_files}
...
\begin{description}
\ourmoditem{add\_libraries(+DirectoryNameList)}{add\_libraries/1}{xsb\_lint}
%
...
\begin{description}
\ourmoditem{empty\_db(-EmptyPrologDB)}{empty\_db/1}{prolog\_db}
...
\ourmoditem{union\_db(+DB1,+DB2,-DB3)}{union\_db/3}{prolog\_db}
