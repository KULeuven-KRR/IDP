...
\item The first subsection of memory statistics, {\tt
  ...
  process-level tather than thread-specific.  Allocated memory is
  broken into different classes including the following.
\begin{itemize}
...
\item Global stack (heap) and local (environment) stack (see e.g.
 ...
 stacks is allocated as a single unit (per thread) so that each stack
 grows together; information is provided on the current allocation for
 the stacks as well as on the stack sizes themselves.  (See
...
\item SLG unification stack for the calling thread. This stack is used
  ...
  or back out.  This stack is not be reallocated unless extremely
...
\item SLG completion stack for the calling thread.  The completion
  ...
  version-dependent.
%
\item Overall space used for tabling, followed by the amout of that
...
\item Information about the number of tabling operations performed in
 the session by any thread.  Global counts (per-thread) are given
